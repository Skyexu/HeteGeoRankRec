 ...
 * @Author: Skye
 * @Date: 21:59 2018/7/8
 * @Description: 计算元路径 U-P-C-P-U-P 的基于计数的相似度
 */
public class MakeUPCPUP implements MakeMetaPath{
    private static final Log LOG = LogFactory.getLog(MakeUPCPUP.class);
    ...
    public MakeUPCPUP(String upInputDataPath,String pcInputDataPath){
    ...
    private void processPreferenceMatrix(){
        ...
        DenseMatrix upcp = new DenseMatrix(numUsers,numItems);
        for (int userIdx = 0; userIdx < numUsers; userIdx++) {
            ...
            for (int itemIndex = 0; itemIndex < numItems; itemIndex++) {
                ...
                upcp.set(userIdx,itemIndex,value);
            }
        }
        DenseMatrix upcpu = new DenseMatrix(numUsers,numUsers);
        for (int userIdx = 0; userIdx < numUsers; userIdx++) {
            DenseVector userRow = upcp.row(userIdx);
            for (int userIdx2 = 0; userIdx2 < numUsers; userIdx2++) {
                ...
                upcpu.set(userIdx,userIdx2,value);
            }
        }
        for (int userIdx = 0; userIdx < numUsers; userIdx++) {
            DenseVector userRow = upcpu.row(userIdx);
            for (int itemIndex = 0; itemIndex < numItems; itemIndex++) {
                SparseVector venueColumn = UPMatrix.column(itemIndex);
                double value =  userRow.inner(venueColumn);
